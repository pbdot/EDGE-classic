...
local function doom_key(x, y, card, skull, card_pic, skull_pic, both_pic)
...
local function turn_digit()
    local r = math.rint(math.rand_range(0, 2))     --always between 0 and 2
...
local function set_face_params()
    -- This routine handles the face states and their timing.
    -- The precedence of expressions is:
    --
    --    dead > evil grin > turned head > straight ahead
...
local function doomguy_face(x, y)
    ...
    if (not face_image or face_tic == sys.gametic or face_state < old_face or (face_state == 3 and old_face == 3)) then
        set_face_params()
    end
...
local function doom_little_ammo()
...
local function doom_status_bar2()
    --Draw our extenders first, just in case the statusbar is already widescreen
    hud.draw_image(-83, 168, "STBARL")     -- Widescreen border
    hud.draw_image(320, 168, "STBARR")     -- Widescreen border
...
local function doom_overlay_status()
    hud.text_font("BIG_DIGIT")
...
local function new_overlay_status()
    local RelX
...
local function edge_draw_bar(BarPos, BarHeight, BarLength, MaxValue, CurrentValue, BarColor1, BarColor2)
    local BarValue = 0
    local percentvalue = 0

    local TopX = BarPos * '1 0 0'
    local TopY = BarPos * '0 1 0'
    ...
    if (BarValue > 1) then
        hud.gradient_box(TopX + 1, TopY + 1, BarValue - 1, BottomBarHeight, BarColor1, BarColor2, BarColor1,
            BarColor2)
...
local function edge_air_bar()
    ...
    local BarLocation = vec3(0.7, 10, 0)
...
local function DoesNameStartWith(TheName, ThePart)
    local tempstr
