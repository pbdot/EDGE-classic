...
{
    if (strncmp(name, "LIN_", 4) == 0)
        return KCTX_Line;
    if (strncmp(name, "SEC_", 4) == 0)
        return KCTX_Sector;
    if (strncmp(name, "TH_", 3) == 0)
        return KCTX_Thing;
    if (strncmp(name, "VT_", 3) == 0)
        return KCTX_Vertex;
...
const char *M_KeyToString(keycode_t key)
{
    ...
    if ((key & MOD_ALL_MASK) == MOD_SHIFT && (key & FL_KEY_MASK) < 127 && isalpha(key & FL_KEY_MASK))
    {
...
{
    if (y_stricmp(str, "browser") == 0)
        return KCTX_Browser;
    if (y_stricmp(str, "render") == 0)
        return KCTX_Render;
    if (y_stricmp(str, "general") == 0)
        return KCTX_General;

    if (y_stricmp(str, "line") == 0)
        return KCTX_Line;
    if (y_stricmp(str, "sector") == 0)
        return KCTX_Sector;
    if (y_stricmp(str, "thing") == 0)
        return KCTX_Thing;
    if (y_stricmp(str, "vertex") == 0)
        return KCTX_Vertex;
...
fprintf(stderr, "REMOVED BINDING key:%04x (%s)\n", temp.key, tokens[0]);
    ...
    if (temp.cmd->req_context != KCTX_NONE && temp.context != temp.cmd->req_context)
    {
...
static bool BindingExists(std::vector<key_binding_t> &list, key_binding_t &bind, bool full_match)
{
...
{
    std::sort(pref_binds.begin(), pref_binds.end(), KeyBind_CMP_pred(column));
...
void M_DetectConflictingBinds()
{
    ...
    {
        if (!(list[k].key == list[k + 1].key && list[k].context == list[k + 1].context))
            continue;
        ...
        {
            if (pref_binds[n].key == list[k].key && pref_binds[n].context == list[k].context)
            {
...
const char *M_StringForBinding(int index, bool changing_key)
{
    ...
    if ((tempk & MOD_ALL_MASK) == MOD_SHIFT && (tempk & FL_KEY_MASK) < 127 && isalpha(tempk & FL_KEY_MASK))
    {
        tempk = toupper(tempk & FL_KEY_MASK);
    }

    snprintf(buffer, sizeof(buffer), "%s%6.6s%-10.10s %-9.9s %.32s", bind.is_duplicate ? "@C1" : "",
             changing_key ? "<?" : ModName_Space(tempk), changing_key ? "\077?>" : BareKeyName(tempk & FL_KEY_MASK),
             ctx_name, M_StringForFunc(index));
#else
    buffer[0] = 0;
#endif
...
static const char *DoParseBindingFunc(key_binding_t &bind, const char *func_str)
{
    ...
    if (cmd->req_context != KCTX_NONE && bind.context != cmd->req_context)
    {
        char *mode = StringUpper(M_KeyContextString(cmd->req_context));

        snprintf(error_msg, sizeof(error_msg), "%s can only be used in %s mode", tokens[0], mode);
...
const char *M_SetLocalBinding(int index, keycode_t key, key_context_e context, const char *func_str)
{
    SYS_ASSERT(0 <= index && index < (int)pref_binds.size());
...
const char *M_AddLocalBinding(int after, keycode_t key, key_context_e context, const char *func_str)
{
    key_binding_t temp;
...
keycode_t M_TranslateKey(int key, int state)
{
    ...
    if (state & MOD_COMMAND)
        key |= MOD_COMMAND;
    else if (state & MOD_META)
        key |= MOD_META;
    else if (state & MOD_ALT)
        key |= MOD_ALT;
...
bool ExecuteCommand(const editor_command_t *cmd, const char *param1, const char *param2, const char *param3,
                    const char *param4)
{
    for (int p = 0; p < MAX_EXEC_PARAM; p++)
    {
    ...
    if (param1[0] == '/')
        EXEC_Flags[f_idx++] = param1;
    else if (param1[0])
        EXEC_Param[p_idx++] = param1;

    if (param2[0] == '/')
        EXEC_Flags[f_idx++] = param2;
    else if (param2[0])
        EXEC_Param[p_idx++] = param2;

    if (param3[0] == '/')
        EXEC_Flags[f_idx++] = param3;
    else if (param3[0])
        EXEC_Param[p_idx++] = param3;

    if (param4[0] == '/')
        EXEC_Flags[f_idx++] = param4;
    else if (param4[0])
        EXEC_Param[p_idx++] = param4;
...
bool ExecuteCommand(const char *name, const char *param1, const char *param2, const char *param3, const char *param4)
{
