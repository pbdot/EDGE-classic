...
static void MergeSandwichLines(int ld1, int ld2, int v, selection_c &del_lines)
{
    ...
    int new_mid_tex = (ld1_onesided) ? L1->Right()->mid_tex : (ld2_onesided) ? L2->Right()->mid_tex : 0;
...
static void DoMergeVertex(int v1, int v2, selection_c &del_lines)
{
    ...
    for (int n = 0; n < NumLineDefs; n++)
    {
        ...
        for (int k = 0; k < NumLineDefs; k++)
        {
            ...
            if ((K->start == v3 && K->end == v2) || (K->start == v2 && K->end == v3))
            {
                found = k;
                break;
            }
...
bool Vertex_TryFixDangler(int v_num)
{
    ...
    for (int i = 0; i < NumVertices; i++)
    {
        ...
        if (abs(dx) <= max_dist && abs(dy) <= max_dist && !LineDefAlreadyExists(v_num, v_other))
        {
            v_other = i;
            break;
        }
...
static double WeightForVertex(const Vertex *V, /* bbox: */ double x1, double y1, double x2, double y2, double width,
...
static double BiggestGapAngle(std::vector<vert_along_t> &along_list, unsigned int *start_idx)
{
    double best_diff = 0;
    double best_low  = 0;
...
static double EvaluateCircle(double mid_x, double mid_y, double r, std::vector<vert_along_t> &along_list,
                             unsigned int start_idx, double arc_rad, double ang_offset /* radians */,
...
void CMD_VT_ShapeArc(void)
{
    ...
    else
    {
        ...
        for (int pos = 0; pos < 1000; pos++)
        {
            ...
            double cost = EvaluateCircle(mid_x, mid_y, r, along_list, start_idx, arc_rad, ang_offset, false);
    ...
    EvaluateCircle(mid_x, mid_y, r, along_list, start_idx, arc_rad, best_offset, true);
