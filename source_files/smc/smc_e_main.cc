...
static void UpdatePanel()
{
    ...
    if (edit.highlight.valid() && edit.highlight.type != edit.mode)
        return;
...
void UpdateDrawLine()
{
    ...
    if (edit.draw_from.valid() && edit.draw_from == edit.highlight)
    {
...
static void UpdateSplitLine(double map_x, double map_y)
{
    ...
    if (edit.mode == OBJ_VERTICES && edit.pointer_in_window && edit.highlight.is_nil())
    {
        FindSplitLine(edit.split_line, edit.split_x, edit.split_y, map_x, map_y, edit.dragged.num);
...
void UpdateHighlight()
{
    ...
    if (edit.pointer_in_window && (edit.action != ACT_DRAG || (edit.mode == OBJ_VERTICES && edit.dragged.valid())))
    {
        GetNearObject(edit.highlight, edit.mode, edit.map_x, edit.map_y);

        // guarantee that we cannot drag a vertex onto itself
        if (edit.action == ACT_DRAG && edit.dragged.valid() && edit.highlight.valid() &&
        ...
        if (grid.ratio > 0 && edit.action == ACT_DRAW_LINE && edit.mode == OBJ_VERTICES && edit.highlight.valid())
        {
            ...
            if (MakeValidCoord(vx) != V->raw_x || MakeValidCoord(vy) != V->raw_y)
            {
                edit.highlight.clear();
            }
        }
...
void Editor_ChangeMode(char mode_char)
{
    ...
    {
    case 't':
        Editor_ChangeMode_Raw(OBJ_THINGS);
        break;
    case 'l':
        Editor_ChangeMode_Raw(OBJ_LINEDEFS);
        break;
    case 's':
        Editor_ChangeMode_Raw(OBJ_SECTORS);
        break;
    case 'v':
        Editor_ChangeMode_Raw(OBJ_VERTICES);
        break;
...
{
    if (type == edit.Selected->what_type() && objnum <= edit.Selected->max_obj())
    {
        invalidated_selection = true;
    }

    if (last_Sel && type == last_Sel->what_type() && objnum <= last_Sel->max_obj())
    {
...
void ConvertSelection(selection_c *src, selection_c *dest)
{
    ...
    if (src->what_type() == OBJ_SECTORS && dest->what_type() == OBJ_LINEDEFS)
    {
        for (int l = 0; l < NumLineDefs; l++)
        {
            ...
            if ((L->Right() && src->get(L->Right()->sector)) || (L->Left() && src->get(L->Left()->sector)))
            {
                dest->set(l);
            }
        }
    ...
    for (l = 0; l < NumLineDefs; l++)
    {
        ...
        if (L->Right())
            dest->set(L->Right()->sector);
        if (L->Left())
            dest->set(L->Left()->sector);
    }
    ...
    for (l = 0; l < NumLineDefs; l++)
    {
        ...
        if (L->Right())
            dest->clear(L->Right()->sector);
        if (L->Left())
            dest->clear(L->Left()->sector);
    }
}
...
void SelectObjectsInBox(selection_c *list, int objtype, double x1, double y1, double x2, double y2)
{
    ...
    case OBJ_LINEDEFS:
        for (int n = 0; n < NumLineDefs; n++)
        {
            ...
            if (x1 <= L->Start()->x() && L->Start()->x() <= x2 && y1 <= L->Start()->y() && L->Start()->y() <= y2 &&
                x1 <= L->End()->x() && L->End()->x() <= x2 && y1 <= L->End()->y() && L->End()->y() <= y2)
            {
                list->toggle(n);
            }
        }
...
Recently_used::Recently_used() : size(0), keep_num(RECENTLY_USED_MAX - 2)
{
...
void Editor_WriteUser(FILE *fp)
{
    ...
    {
    case OBJ_THINGS:
        fprintf(fp, "edit_mode t\n");
        break;
    case OBJ_LINEDEFS:
        fprintf(fp, "edit_mode l\n");
        break;
    case OBJ_SECTORS:
        fprintf(fp, "edit_mode s\n");
        break;
    case OBJ_VERTICES:
        fprintf(fp, "edit_mode v\n");
        break;
