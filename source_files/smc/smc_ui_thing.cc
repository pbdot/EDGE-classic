...
class thing_opt_CB_data_c
{
  ...
  public:
    thing_opt_CB_data_c(UI_ThingBox *_parent, int _mask) : parent(_parent), mask(_mask)
    {
    }
...
const char **arrow_pixmaps[8] = {arrow_0_xpm,   arrow_45_xpm,  arrow_90_xpm,  arrow_135_xpm,
                                 arrow_180_xpm, arrow_225_xpm, arrow_270_xpm, arrow_315_xpm};
...
int UI_ThingBox::CalcOptions() const
{
    ...
    if (Features.strife_flags)
    {
        if (o_sf_ambush->value())
            options |= MTF_Strife_Ambush;
        if (o_sf_friend->value())
            options |= MTF_Strife_Friend;
        if (o_sf_shadow->value())
            options |= MTF_Strife_Shadow;
        if (o_sf_altvis->value())
            options |= MTF_Strife_AltVis;
        if (o_sf_stand->value())
            options |= MTF_Strife_Stand;
    }
    else
    {
        if (o_ambush->value())
    ...
    {
        if (o_sp->value())
            options |= MTF_Hexen_SP;
        if (o_coop->value())
            options |= MTF_Hexen_COOP;
        if (o_dm->value())
            options |= MTF_Hexen_DM;

        if (o_fight->value())
            options |= MTF_Hexen_Fighter;
        if (o_cleric->value())
            options |= MTF_Hexen_Cleric;
        if (o_mage->value())
            options |= MTF_Hexen_Mage;

        if (o_dormant->value())
            options |= MTF_Hexen_Dormant;
    }
...
{
#ifdef _FLTK_DISABLED
    if (field < 0 || field == Thing::F_X || field == Thing::F_Y || field == Thing::F_H)
    {
        if (is_thing(obj))
        {
...
void UI_ThingBox::UpdateGameInfo()
{
    ...
    if (Level_format != MAPF_Doom)
    {
        ...
        ocb       = (thing_opt_CB_data_c *)o_sp->user_data();
        ocb->mask = MTF_Hexen_SP;
        ocb       = (thing_opt_CB_data_c *)o_coop->user_data();
        ocb->mask = MTF_Hexen_COOP;
        ocb       = (thing_opt_CB_data_c *)o_dm->user_data();
        ocb->mask = MTF_Hexen_DM;
    }
    else
    {
        pos_z->hide();
