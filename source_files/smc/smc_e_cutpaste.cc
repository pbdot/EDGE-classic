...
class clipboard_data_c
{
  ...
  public:
    clipboard_data_c(obj_type_e _mode)
        : mode(_mode), uses_real_sectors(false), things(), verts(), sectors(), sides(), lines()
    {
...
static void ReselectGroup()
{
    ...
    {
        if (clip_board->mode == OBJ_THINGS || clip_board->mode == OBJ_SECTORS)
        {
    ...
    bool was_mappy =
        (clip_board->mode == OBJ_VERTICES || clip_board->mode == OBJ_LINEDEFS || clip_board->mode == OBJ_SECTORS);

    bool is_mappy = (edit.mode == OBJ_VERTICES || edit.mode == OBJ_LINEDEFS || edit.mode == OBJ_SECTORS);
    ...
    for (int n = 0; n < NumLineDefs; n++)
    {
        ...
        if (lines->get(n) || verts->get(L->start) || verts->get(L->end))
        {
            ...
            if (L->WhatSector(SIDE_LEFT) >= 0)
                result->set(L->WhatSector(SIDE_LEFT));
            if (L->WhatSector(SIDE_RIGHT) >= 0)
                result->set(L->WhatSector(SIDE_RIGHT));
        }
    }
...
void DeleteObjects_WithUnused(selection_c *list, bool keep_things, bool keep_verts, bool keep_lines)
{
    selection_c vert_sel(OBJ_VERTICES);
    selection_c side_sel(OBJ_SIDEDEFS);
    selection_c line_sel(OBJ_LINEDEFS);
