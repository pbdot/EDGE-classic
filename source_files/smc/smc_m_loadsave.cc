...
void CMD_FreshMap()
{
    ...
    {
        if (DLG_Confirm("Cancel|&Overwrite", overwrite_message, "current") <= 0)
        {
            return;
        }
    }
...
{
    if (ld->start >= NumVertices || ld->end >= NumVertices || ld->start == ld->end)
    {
...
static void LoadSideDefs()
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (raw.upper_tex[0] == 0)
            strcpy(raw.upper_tex, "-");
        if (raw.lower_tex[0] == 0)
            strcpy(raw.lower_tex, "-");
        if (raw.mid_tex[0] == 0)
            strcpy(raw.mid_tex, "-");
...
bool M_SaveMap()
{
    ...
    if (edit_wad->IsReadOnly())
    {
        if (DLG_Confirm("Cancel|&Export", "The current pwad is a READ-ONLY file. "
...
bool M_ExportMap()
{
    ...
    {
        if (DLG_Confirm("Cancel|&Overwrite", overwrite_message, "selected") <= 0)
        {
            delete wad;
            return false;
        }
...
void CMD_DeleteMap()
{
    ...
    {
        // perhaps ask either to Rename map, or Delete the file (and Eureka will
        // shut down)
    ...
    if (DLG_Confirm("Cancel|&Delete", "Are you sure you want to delete this map? "
