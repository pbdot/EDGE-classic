...
//
static inline void ComputeIntersection(seg_t *seg, seg_t *part, double perp_c, double perp_d, double *x, double *y)
{
    double ds;
...
static void AddIntersection(intersection_t **cut_list, vertex_t *vert, seg_t *part, bool self_ref)
{
...
//
static int EvalPartitionWorker(quadtree_c *tree, seg_t *part, int best_cost, eval_info_t *info)
{
    double qnty;
    double a, b, fa, fb;
...
static int EvalPartition(quadtree_c *tree, seg_t *part, int best_cost)
{
    ...
    DebugPrintf("Eval %p: splits=%d iffy=%d near=%d left=%d+%d right=%d+%d "
                "cost=%d.%02d\n",
                part, info.splits, info.iffy, info.near_miss, info.real_left, info.mini_left, info.real_right,
                info.mini_right, info.cost / 100, info.cost % 100);
#endif

    return info.cost;
}

static void EvaluateFastWorker(quadtree_c *tree, seg_t **best_H, seg_t **best_V, int mid_x, int mid_y)
{
    seg_t *part;
...
static bool PickNodeWorker(quadtree_c *part_list, quadtree_c *tree, seg_t **best, int *best_cost)
{
    // try each partition
...
seg_t *PickNode(quadtree_c *tree, int depth)
{
    ...
    else
    {
        DebugPrintf("PickNode: Best has score %d.%02d  (%1.1f,%1.1f) -> (%1.1f,%1.1f)\n", best_cost / 100,
                    best_cost % 100, best->start->x, best->start->y, best->end->x, best->end->y);
    }
#endif
...
static void SeparateSegs(quadtree_c *tree, seg_t *part, seg_t **left_list, seg_t **right_list,
...
void AddMinisegs(intersection_t *cut_list, seg_t *part, seg_t **left_list, seg_t **right_list)
{
    ...
    {
        DebugPrintf("  Vertex %8X (%1.1f,%1.1f)  Along %1.2f  [%d/%d]  %s\n", cur->vertex->index, cur->vertex->x,
                    cur->vertex->y, cur->along_dist, cur->open_before ? 1 : 0, cur->open_after ? 1 : 0,
...
static seg_t *CreateOneSeg(int line, vertex_t *start, vertex_t *end, int sidedef, int what_side /* 0 or 1 */)
{
    SideDef *sd = NULL;
    if (sidedef >= 0)
        sd = SideDefs[sidedef];
...
static void DebugShowSegs(seg_t *list)
{
    for (; list != NULL; list = list->next)
    {
        ...
        DebugPrintf("Build:   %sSEG %p  linedef=%d  (%1.1f,%1.1f) -> (%1.1f,%1.1f)\n",
                    (seg->linedef >= 0) ? "" : "MINI", seg, seg->linedef, seg->start->x, seg->start->y, seg->end->x,
                    seg->end->y);
    }
}
#endif
...
void subsec_t::RoundOff()
{
    ...
    if (real_total == 0)
    {
        ...
        DebugPrintf("Degenerate after:  (%d,%d) -> (%d,%d)\n", I_ROUND(last_real_degen->start->x),
                    I_ROUND(last_real_degen->start->y), I_ROUND(last_real_degen->end->x),
