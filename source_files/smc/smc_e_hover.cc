...
#include "smc_e_linedef.h" // SplitLineDefAtVertex()
...
double ApproxDistToLineDef(const LineDef *L, double x, double y)
{
    ...
    else
    {
        ...
        if (y < (dy > 0 ? y1 : y2))
            return hypot(x - (dy > 0 ? x1 : x2), y - (dy > 0 ? y1 : y2));

        if (y > (dy > 0 ? y2 : y1))
            return hypot(x - (dy > 0 ? x2 : x1), y - (dy > 0 ? y2 : y1));
...
class fastopp_node_c
{
  ...
  public:
    fastopp_node_c(int _low, int _high) : lo(_low), hi(_high), lo_child(NULL), hi_child(NULL), lines()
    {
        mid = (lo + hi) / 2;
  ...
  public:
    ...
    {
        if (lo_child && (x1 > lo_child->lo) && (x2 < lo_child->hi))
        {
            lo_child->AddLine_X(ld, x1, x2);
            return;
        }

        if (hi_child && (x1 > hi_child->lo) && (x2 < hi_child->hi))
        {
    ...
    {
        if (lo_child && (y1 > lo_child->lo) && (y2 < lo_child->hi))
        {
            lo_child->AddLine_Y(ld, y1, y2);
            return;
        }

        if (hi_child && (y1 > hi_child->lo) && (y2 < hi_child->hi))
        {
...
{
    delete fastopp_X_tree;
    fastopp_X_tree = NULL;
    delete fastopp_Y_tree;
    fastopp_Y_tree = NULL;
}
...
static Objid NearestLineDef(double x, double y)
{
    ...
    for (int n = 0; n < NumLineDefs; n++)
    {
        ...
        if (MAX(x1, x2) < lx || MIN(x1, x2) > hx || MAX(y1, y2) < ly || MIN(y1, y2) > hy)
            continue;
...
static Objid NearestSector(double x, double y)
{
    ...
    else if (line1 < 0 || ApproxDistToLineDef(LineDefs[line2], x, y) < ApproxDistToLineDef(LineDefs[line1], x, y))
    {
        line1 = line2;
        side1 = side2;
    }
...
static Objid NearestThing(double x, double y)
{
    ...
    for (int n = 0; n < NumThings; n++)
    {
        ...
        if (x < tx - r - mapslack || x > tx + r + mapslack || y < ty - r - mapslack || y > ty + r + mapslack)
            continue;
...
void FindSplitLine(Objid &out, double &out_x, double &out_y, double ptr_x, double ptr_y, int ignore_vert)
{
    out_x = out_y = 0;
...
static void FindCrossingLines(crossing_state_c &cross, double x1, double y1, int possible_v1, double x2, double y2,
                              int possible_v2)
{
    ...
    for (int ld = 0; ld < NumLineDefs; ld++)
    {
        ...
        if (MAX(lx1, lx2) < bbox_x1 || MIN(lx1, lx2) > bbox_x2 || MAX(ly1, ly2) < bbox_y1 || MIN(ly1, ly2) > bbox_y2)
        {
            continue;
        }
        ...
        if (!((a < -CROSSING_EPSILON && b > CROSSING_EPSILON) || (a > CROSSING_EPSILON && b < -CROSSING_EPSILON)))
        {
            continue;
        }
                        ...
                        int possible_v2)
{
    ...
    for (int v = 0; v < NumVertices; v++)
    {
        ...
        if (VC->Matches(TO_COORD(x1), TO_COORD(y1)) || VC->Matches(TO_COORD(x2), TO_COORD(y2)))
            continue;
    ...
    for (size_t k = 0; k < num_verts; k++)
    {
        ...
        FindCrossingLines(cross, cur_x1, cur_y1, cur_v, next_x2, next_y2, next_v);
