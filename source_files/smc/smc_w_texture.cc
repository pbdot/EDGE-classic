...
#include "smc_m_game.h" /* yg_picture_format */
#include "smc_w_loadpic.h"
...
static bool CheckTexturesAreStrife(byte *tex_data, int tex_length, int num_tex, bool skip_first)
{
...
static void LoadTextureEntry_Strife(byte *tex_data, int tex_length, int offset, byte *pnames, int pname_size,
                                    bool skip_first)
{
    ...
    for (int j = 0; j < num_patches; j++, patdef++)
    {
        ...
        if (!lump || !LoadPicture(*img, lump, picname, xofs, yofs))
        {
...
static void LoadTextureEntry_DOOM(byte *tex_data, int tex_length, int offset, byte *pnames, int pname_size,
                                  bool skip_first)
{
    const raw_texture_t *raw = (const raw_texture_t *)(tex_data + offset);
...
static void LoadTexturesLump(Lump_c *lump, byte *pnames, int pname_size, bool skip_first)
{
