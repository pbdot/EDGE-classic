...
sector_subdivision_c::sector_subdivision_c() : polygons()
{
}
...
void sector_subdivision_c::AddPolygon(float lx1, float lx2, float low_y, float hx1, float hx2, float high_y)
{
  ...
  public:
    ...
    void CheckSlopeThing(const Thing *T)
    {
        if (Level_format != MAPF_Doom && (Features.slopes & 16))
        {
            ...
            case 9502:
                PlaneTiltByThing(T, 0);
                break;
            case 9503:
                PlaneTiltByThing(T, 1);
                break;
            default:
                break;
            }
    ...
    void CheckSlopeCopyThing(const Thing *T)
    {
        if (Level_format != MAPF_Doom && (Features.slopes & 16))
        {
            ...
            {
            case 9510:
                PlaneCopyFromThing(T, 0);
                break;
            case 9511:
                PlaneCopyFromThing(T, 1);
                break;
            default:
                break;
            }
    ...
    void PlaneAlign(const LineDef *L, int floor_mode, int ceil_mode)
    {
        ...
        {
        case 1:
            PlaneAlignPart(L, SIDE_RIGHT, 0 /* floor */);
            break;
        case 2:
            PlaneAlignPart(L, SIDE_LEFT, 0);
            break;
        default:
            break;
        }
        ...
        {
        case 1:
            PlaneAlignPart(L, SIDE_RIGHT, 1 /* ceil */);
            break;
        case 2:
            PlaneAlignPart(L, SIDE_LEFT, 1);
            break;
        default:
            break;
        }
    ...
    void PlaneAlignPart(const LineDef *L, int side, int plane)
    {
        ...
        if (plane > 0)
        { // ceiling
            SlopeFromLine(infos[sec_num].floors.c_plane, lx1, ly1, back->ceilh, vx, vy, front->ceilh);
        }
        else
        { // floor
            SlopeFromLine(infos[sec_num].floors.f_plane, lx1, ly1, back->floorh, vx, vy, front->floorh);
        }
    }
    ...
    void PlaneCopy(const LineDef *L, int f1_tag, int c1_tag, int f2_tag, int c2_tag, int share)
    {
        ...
        if (L->left >= 0 && L->right >= 0)
        {
            ...
            {
            case 1:
                infos[back_sec].floors.f_plane.Copy(infos[front_sec].floors.f_plane);
                break;
            case 2:
                infos[front_sec].floors.f_plane.Copy(infos[back_sec].floors.f_plane);
                break;
            default:
                break;
            }
            ...
            {
            case 4:
                infos[back_sec].floors.c_plane.Copy(infos[front_sec].floors.c_plane);
                break;
            case 8:
                infos[front_sec].floors.c_plane.Copy(infos[back_sec].floors.c_plane);
                break;
            default:
                break;
            }
    ...
    void SlopeFromLine(slope_plane_c &pl, double x1, double y1, double z1, double x2, double y2, double z2)
    {
        double dx = x2 - x1;
        double dy = y2 - y1;
        double dz = z2 - z1;
...
static void R_SubdivideSector(int num, sector_extra_info_t &exinfo)
{
    ...
    */
    ...
    for (int n = exinfo.first_line; n <= exinfo.last_line; n++)
    {
        ...
        /* DEBUG
        fprintf(stderr, "Line %d  mapped coords (%d %d) .. (%d %d)  flipped:%d
    ...
    for (;;)
    {
        ...
        for (unsigned int k = 0; k < active_edges.size(); k++)
        {
            ...
            if (A != NULL)
            {
                ...
                if (A->y1 > low_y)
                    high_y = MIN(high_y, A->y1);
                if (A->y2 > low_y)
                    high_y = MIN(high_y, A->y2);
            }
        }
...
bool Subdiv_SectorOnScreen(int num, double map_lx, double map_ly, double map_hx, double map_hy)
{
    ...
    if (exinfo.bound_x1 > map_hx || exinfo.bound_x2 < map_lx || exinfo.bound_y1 > map_hy || exinfo.bound_y2 < map_ly)
    {
...
sector_3dfloors_c::sector_3dfloors_c() : heightsec(-1)
{
}
