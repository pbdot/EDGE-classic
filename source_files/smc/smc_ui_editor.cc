...
void UI_TextEditor::menu_do_save(Fl_Widget *w, void *data)
{
    ...
    if (win->read_only)
    {
        if (DLG_Confirm("Cancel|&Export", "The current wad is READ-ONLY.  "
...
static Fl_Menu_Item ted_menu_items[] = {{"&File", 0, 0, 0, FL_SUBMENU},
...
#endif
      want_close(false), want_save(false), is_new(true), read_only(false), has_changes(false), find_string(NULL)
{
#ifdef _FLTK_DISABLED
    size_range(520, 200);
...
void UI_TextEditor::close_callback(Fl_Widget *w, void *data)
{
    ...
    if (win->has_changes)
    {
        if (DLG_Confirm("Cancel|&Discard", "You have unsaved changes to this lump.  "
