...
UI_ChooseMap::UI_ChooseMap(const char *initial_name, Wad_file *_rename_wad)
    : UI_Escapable_Window(420, 385, "Choose Map"), rename_wad(_rename_wad), action(ACT_none)
{
...
UI_OpenMap::UI_OpenMap()
    : UI_Escapable_Window(420, 475, "Open Map"), action(ACT_none), loaded_wad(NULL), using_wad(NULL)
{
#ifdef _FLTK_DISABLED
    resizable(NULL);
...
void UI_OpenMap::CheckMapName()
{
    ...
    bool is_valid =
        (using_wad != NULL) && ValidateMapName(map_name->value()) && (using_wad->LevelFind(map_name->value()) >= 0);
...
UI_ProjectSetup::UI_ProjectSetup(bool new_project, bool is_startup)
    : UI_Escapable_Window(400, is_startup ? 200 : 440, new_project ? "New Project" : "Manage Project"),
      action(ACT_none), game(NULL), port(NULL), map_format(MAPF_INVALID), name_space()
{
#ifdef _FLTK_DISABLED
    callback(close_callback, this);
