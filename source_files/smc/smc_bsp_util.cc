...
char *UtilTimeString(void)
{
    ...
    return StringPrintf("%04d-%02d-%02d %02d:%02d:%02d.%04d", sys_time.wYear, sys_time.wMonth, sys_time.wDay,
                        sys_time.wHour, sys_time.wMinute, sys_time.wSecond, sys_time.wMilliseconds * 10);
    ...
    return StringPrintf("%04d-%02d-%02d %02d:%02d:%02d.%04d", calend_time->tm_year + 1900, calend_time->tm_mon + 1,
                        calend_time->tm_mday, calend_time->tm_hour, calend_time->tm_min, calend_time->tm_sec, 0);
#endif
}
...
static void MarkPolyobjSector(int sector)
{
    ...
    for (i = 0; i < NumLineDefs; i++)
    {
        ...
        if ((L->right >= 0 && L->Right()->sector == sector) || (L->left >= 0 && L->Left()->sector == sector))
        {
...
static void MarkPolyobjPoint(double x, double y)
{
    ...
    for (i = 0; i < NumLineDefs; i++)
    {
        ...
        if (CheckLinedefInsideBox(bminx, bminy, bmaxx, bmaxy, (int)L->Start()->x(), (int)L->Start()->y(),
...
static inline int LineVertexLowest(const LineDef *L)
{
    ...
    return (L->Start()->raw_x < L->End()->raw_x ||
            (L->Start()->raw_x == L->End()->raw_x && L->Start()->raw_y < L->End()->raw_y))
               ? 0
               : 1;
}
...
static void VertexAddWallTip(vertex_t *vert, double dx, double dy, int open_left, int open_right)
{
    if (vert->overlap)
        vert = vert->overlap;
