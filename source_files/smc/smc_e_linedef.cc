...
//
static int ScoreAdjoiner(const Objid &adj, const Objid &cur, int align_flags)
{
...
//
static void DetermineAdjoiner(Objid &result, const Objid &cur, int align_flags)
{
    int best_score = 0;
...
static void DoAlignX(const Objid &cur, const Objid &adj, int align_flags)
{
...
static void DoAlignY(const Objid &cur, const Objid &adj, int align_flags)
{
    const LineDef *L  = LD_ptr(cur);
    ...
    if ((align_flags & LINALIGN_Unpeg) && L->TwoSided() && lower_vis && upper_vis && (!lower_unpeg || !upper_unpeg) &&
        (PartialTexCmp(SD->LowerTex(), SD->UpperTex()) == 0) && is_null_tex(SD->MidTex()) /* no rail */)
    {
...
// surface at 'j'.
static bool Align_CheckAdjacent(std::vector<Objid> &group, int j, int k, bool do_right)
{
...
//
static int Align_PickNextSurface(std::vector<Objid> &group, const std::vector<byte> &seen, bool do_right)
{
    int fallback = -1;
...
void CMD_LIN_Align()
{
    ...
    if (do_X)
        align_flags = align_flags | LINALIGN_X;
    if (do_Y)
        align_flags = align_flags | LINALIGN_Y;

    if (do_right)
        align_flags |= LINALIGN_Right;
    if (do_unpeg)
        align_flags |= LINALIGN_Unpeg;
    if (do_clear)
        align_flags |= LINALIGN_Clear;
...
static bool DoSplitLineDef(int ld)
{
    ...
    if (abs(L->Start()->x() - L->End()->x()) < 4 && abs(L->Start()->y() - L->End()->y()) < 4)
        return false;
...
void LineDefs_SetLength(int new_len)
{
    ...
    for (int n = 0; n < NumLineDefs; n++)
    {
        ...
        angles[n] = ComputeAngle(L->End()->x() - L->Start()->x(), L->End()->y() - L->Start()->y());
    }
