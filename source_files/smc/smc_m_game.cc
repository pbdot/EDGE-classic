...
#else
#define UNKNOWN_THING_COLOR 0x00FFFF
#endif
...
GameInfo_c::GameInfo_c(std::string _name) : name(_name), base_game()
{
}
...
PortInfo_c::PortInfo_c(std::string _name) : name(_name), formats(0), supported_games(), udmf_namespace()
{
}
...
class parser_state_c
{
  ...
  public:
    parser_state_c() : lineno(0), fname(NULL), argc(0), cond_stack(), current_gen_line(-1)
    {
...
static const char *const bad_arg_count = "%s(%d): directive \"%s\" takes %d parameters\n";
...
static void M_ParseNormalLine(parser_state_c *pst)
{
    ...
    if (y_stricmp(argv[0], "base_game") == 0 || y_stricmp(argv[0], "map_formats") == 0 ||
        y_stricmp(argv[0], "supported_games") == 0 || y_stricmp(argv[0], "udmf_namespace") == 0)
    {
        return;
    }
    ...
    else if (y_stricmp(argv[0], "linegroup") == 0 || y_stricmp(argv[0], "spec_group") == 0)
    {
        if (nargs != 2)
            FatalError(bad_arg_count, pst->fname, pst->lineno, argv[0], 2);
    ...
    else if (y_stricmp(argv[0], "line") == 0 || y_stricmp(argv[0], "special") == 0)
    {
        if (nargs < 3)
            FatalError(bad_arg_count, pst->fname, pst->lineno, argv[0], 3);
...
static void M_ParseGameInfoLine(parser_state_c *pst)
{
    ...
    if (y_stricmp(argv[0], "map_formats") == 0 || y_stricmp(argv[0], "supported_games") == 0 ||
...
//
void M_ParseDefinitionFile(parse_purpose_e purpose, const char *filename, const char *folder, const char *prettyname,
                           int include_level)
{
    if (!folder)
        folder = "common";
    ...
    while (M_ReadTextLine(pst->readbuf, sizeof(pst->readbuf), fp))
    {
        ...
        if (y_stricmp(pst->argv[0], "include") == 0)
        {
            ...
            M_ParseDefinitionFile(purpose, new_name, new_folder, NULL /* prettyname */, include_level + 1);
            continue;
        }
    ...
    if (!pst->cond_stack.empty())
    {
        FatalError("%s(%d): Missing endif statement\n", pst->fname, pst->cond_stack.back().start_line);
    }
...
void M_CollectKnownDefs(const char *folder, std::vector<const char *> &list)
{
    ...
    {
        if (pos + 1 < temp_list.size() && y_stricmp(temp_list[pos], temp_list[pos + 1]) == 0)
        {
...
const sectortype_t *M_GetSectorType(int type)
{
    ...
    static sectortype_t dummy_type = {"UNKNOWN TYPE"};
...
const linetype_t *M_GetLineType(int type)
{
    ...
    static linetype_t dummy_type = {0, "UNKNOWN TYPE"};
...
const thingtype_t *M_GetThingType(int type)
{
    ...
    static thingtype_t dummy_type = {0, 0, UNKNOWN_THING_RADIUS, 1.0f, "UNKNOWN TYPE", "NULL", UNKNOWN_THING_COLOR};
