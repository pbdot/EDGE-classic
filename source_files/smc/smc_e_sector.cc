...
lineloop_c::lineloop_c() : lines(), sides(), faces_outward(false), islands()
{
}
...
bool TraceLineLoop(int ld, int side, lineloop_c &loop, bool ignore_bare)
{
    ...
    if (Vertex_HowManyLineDefs(cur_vert) == 1 && Vertex_HowManyLineDefs(prev_vert) == 1)
        return false;
...
bool lineloop_c::LookForIsland()
{
    ...
    for (int ld = 0; ld < NumLineDefs; ld++)
    {
        ...
        if (MAX(x1, x2) < bbox_x1 || MIN(x1, x2) > bbox_x2 || MAX(y1, y2) < bbox_y1 || MIN(y1, y2) > bbox_y2)
            continue;
...
void lineloop_c::Dump() const
{
    ...
    for (unsigned int i = 0; i < lines.size(); i++)
    {
        ...
        DebugPrintf("  %s of line #%d : (%d %d) --> (%d %d)\n", sides[i] == SIDE_LEFT ? " LEFT" : "RIGHT", lines[i],
...
static void DetermineNewTextures(lineloop_c &loop, std::vector<int> &lower_texs, std::vector<int> &upper_texs)
{
    ...
    for (pass = 0; pass < 2; pass++)
    {
        for (k = 0; k < total; k++)
        {
            ...
            {
                if (SD->lower_tex == null_tex)
                    continue;
                if (SD->upper_tex == null_tex)
                    continue;
    ...
    for (k = 0; k < total; k++)
    {
        ...
        if (lower_texs[k] == null_tex)
            lower_texs[k] = def_lower;
        if (upper_texs[k] == null_tex)
            upper_texs[k] = def_upper;
    }
    ...
    for (pass = 0; pass < total * 2; pass++)
    {
        for (k = 0; k < total; k++)
        {
            ...
            if (p < total && lower_texs[p] < 0)
                p = total;
            if (n < total && lower_texs[n] < 0)
                n = total;
...
//
void DoAssignSector(int ld, int side, int new_sec, int new_lower, int new_upper, selection_c *flip)
{
...
void lineloop_c::AssignSector(int new_sec, selection_c *flip)
{
    ...
    for (unsigned int k = 0; k < lines.size(); k++)
    {
        DoAssignSector(lines[k], sides[k], new_sec, lower_texs[k], upper_texs[k], flip);
    }
...
bool AssignSectorToSpace(double map_x, double map_y, int new_sec, int model)
{
    ...
    for (int n = 0; n < NumLineDefs; n++)
    {
        ...
        if (L->WhatSector(SIDE_LEFT) >= 0)
            unused.clear(L->WhatSector(SIDE_LEFT));
        if (L->WhatSector(SIDE_RIGHT) >= 0)
            unused.clear(L->WhatSector(SIDE_RIGHT));
    }
