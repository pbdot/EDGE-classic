...
#include "smc_im_color.h" /* trans_replace */
...
static bool ComposePicture(Img_c &dest, Img_c *sub, int pic_x_offset, int pic_y_offset, int *pic_width, int *pic_height)
{
    if (sub == NULL)
        return false;
...
char W_DetectImageFormat(Lump_c *lump)
{
    ...
    if (width > 0 && width <= 2048 && abs(ofs_x) <= 2048 && height > 0 && height <= 512 && abs(ofs_y) <= 512 &&
