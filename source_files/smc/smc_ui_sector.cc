...
int headroom_presets[UI_SectorBox::HEADROOM_BUTTONS] = {0, 64, 96, 128, 192, 256};
...
void UI_SectorBox::SetSectorType(int new_type)
{
    ...
    int mask = (Features.gen_sectors == 2) ? 255 : (Features.gen_sectors == 1) ? 31 : 65535;
...
{
    return (f_pic->Selected() ? PART_FLOOR : 0) | (c_pic->Selected() ? PART_CEIL : 0);
}

int UI_SectorBox::GetHighlightedPics() const
{
    return (f_pic->Highlighted() ? PART_FLOOR : 0) | (c_pic->Highlighted() ? PART_CEIL : 0);
}
...
void UI_SectorBox::CB_Cut(int parts)
{
    ...
    if (!edit.Selected->empty())
    {
        ...
        {
            if (parts & PART_FLOOR)
                BA_ChangeSEC(*it, Sector::F_FLOOR_TEX, new_floor);
            if (parts & PART_CEIL)
                BA_ChangeSEC(*it, Sector::F_CEIL_TEX, new_ceil);
        }
