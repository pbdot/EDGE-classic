...
static void Navigate2D()
{
    ...
    if (edit.nav_left || edit.nav_right || edit.nav_up || edit.nav_down)
    {
...
#ifdef _FLTK_DISABLED
static void ParseOperationLine(const char **tokens, int num_tok, Fl_Menu_Button *menu)
{
    // just a divider?
...
void CMD_OperationMenu()
{
    ...
    if (item)
    {
        ...
        ExecuteCommand(info->cmd, info->param[0], info->param[1], info->param[2], info->param[3]);
    }
#endif
}
