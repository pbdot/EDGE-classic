...
bool OtherLineDef(int L, int V, int *L_other, int *V_other, int match, int start_L)
{
    *L_other = -1;
    *V_other = -1;
...
void CMD_LIN_SelectPath(void)
{
    ...
    if (Exec_HasFlag("/onesided"))
        match |= SLP_OneSided;
    if (Exec_HasFlag("/sametex"))
        match |= SLP_SameTex;
...
static bool GrowContiguousSectors(selection_c &seen)
{
    ...
    for (int n = 0; n < NumLineDefs; n++)
    {
        ...
        if (do_floor_h && (S1->floorh != S2->floorh))
            continue;
        if (do_ceil_h && (S1->ceilh != S2->ceilh))
            continue;

        if (do_floor_tex && (S1->floor_tex != S2->floor_tex))
            continue;
        if (do_ceil_tex && (S1->ceil_tex != S2->ceil_tex))
            continue;

        if (do_light && (S1->light != S2->light))
            continue;
        if (do_tag && (S1->tag != S2->tag))
            continue;
        if (do_special && (S1->type != S2->type))
            continue;
...
static void CalcPropagation(std::vector<byte> &vec, bool ignore_doors)
{
    ...
    do
    {
        ...
        for (int n = 0; n < NumLineDefs; n++)
        {
            ...
            // check for doors
            if (!ignore_doors &&
                (MIN(Sectors[sec1]->ceilh, Sectors[sec2]->ceilh) <= MAX(Sectors[sec1]->floorh, Sectors[sec2]->floorh)))
            {
                continue;
            }
...
static void CalcFinalPropagation()
{
    for (int s = 0; s < NumSectors; s++)
    {
        ...
        {
        case 0:
            sound_prop_vec[s] = PGL_Never;
            break;
        case 1:
            sound_prop_vec[s] = PGL_Level_1;
            break;
        case 2:
            sound_prop_vec[s] = PGL_Level_2;
            break;
        }
    }
}
...
const byte *SoundPropagation(int start_sec)
{
    ...
    if (sound_propagation_invalid || sound_start_sec != start_sec)
    {
