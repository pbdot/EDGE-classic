...
class recent_file_data_c
{
  ...
  public:
    recent_file_data_c(const char *_file, const char *_map) : file(_file), map(_map)
    {
    }
...
class RecentFiles_c
{
  ...
  public:
    ...
    std::string Format(int index) const
    {
        ...
        char buffer[256];
        snprintf(buffer, sizeof(buffer), "%s%s%d:  %-.42s", (index < 9) ? "  " : "", (index < 9) ? "&" : "", 1 + index,
                 name);
...
#ifdef WIN32
        "c:/doom", "c:/doom2", "c:/doom95",
#else
        "/usr/share/games/doom", "/usr/share/doom", "/usr/local/share/games/doom", "/usr/local/games/doom",
#endif
        NULL};
