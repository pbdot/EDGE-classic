...
UI_MainWindow::UI_MainWindow()
    :
#ifdef _FLTK_DISABLED
      Fl_Double_Window(WINDOW_MIN_W, WINDOW_MIN_H, EUREKA_TITLE), cursor_shape(FL_CURSOR_DEFAULT),
#endif
...
int UI_MainWindow::GetPanelObjNum() const
{
    ...
    {
    case OBJ_THINGS:
        return thing_box->GetObj();
    case OBJ_VERTICES:
        return vert_box->GetObj();
    case OBJ_SECTORS:
        return sec_box->GetObj();
    case OBJ_LINEDEFS:
        return line_box->GetObj();
...
void UI_MainWindow::SetTitle(const char *wad_name, const char *map_name, bool read_only)
{
#ifdef _FLTK_DISABLED
    ...
    if (wad_name)
    {
        ...
        snprintf(title_buf, sizeof(title_buf), "%s (%s)%s", wad_name, map_name, read_only ? " [Read-Only]" : "");
    }
    else
    {
...
UI_LogViewer::UI_LogViewer() : UI_Escapable_Window(580, 400, "Eureka Log Viewer")
{
#ifdef _FLTK_DISABLED
    box(FL_NO_BOX);
