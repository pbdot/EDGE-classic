...
class UI_PortPathDialog : public UI_Escapable_Window
{
  ...
  public:
        ...
        : UI_Escapable_Window(560, 250, "Port Settings"), exe_name(NULL), want_close(false)
    {
        ...
        header = new Fl_Box(FL_NO_BOX, 20, 55, w() - 40, 30, "This is only needed for the Test Map command.");
        header->align(FL_ALIGN_INSIDE | FL_ALIGN_LEFT);
...
static const char *CalcWarpString()
{
    ...
    if (strlen(Level_name) >= 4 && y_strnicmp(Level_name, "MAP", 3) == 0 && isdigit(Level_name[3]))
    {
    ...
    if (strlen(Level_name) >= 4 && !isdigit(Level_name[0]) && isdigit(Level_name[1]) && !isdigit(Level_name[2]) &&
...
static const char *GrabWadNames(const port_path_info_t *info)
{
    ...
    if (edit_wad)
    {
        AppendWadName(wad_names, sizeof(wad_names), edit_wad->PathName(), !has_file ? "-file" : NULL);
    }
...
void CMD_TestMap()
{
    if (MadeChanges)
    {
        if (DLG_Confirm("Cancel|&Save", "You have unsaved changes, do you want to save them now "
    ...
    snprintf(cmd_buffer, sizeof(cmd_buffer), "%s %s %s", CalcEXEName(info), GrabWadNames(info), CalcWarpString());
