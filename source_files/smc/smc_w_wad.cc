...
Lump_c::Lump_c(Wad_file *_par, const char *_nam, int _start, int _len) : parent(_par), l_start(_start), l_length(_len)
{
    name = StringDup(_nam);
...
Lump_c::Lump_c(Wad_file *_par, const struct raw_wad_entry_s *entry) : parent(_par)
{
...
Wad_file::Wad_file(const char *_name, char _mode, FILE *_fp)
    : mode(_mode), fp(_fp), kind('P'), total_size(0), directory(), dir_start(0), dir_count(0), dir_crc(0), levels(),
      patches(), sprites(), flats(), tx_tex(), begun_write(false), insert_point(-1)
{
...
{
    if (y_stricmp(name, "THINGS") == 0)
        return 1;
    if (y_stricmp(name, "LINEDEFS") == 0)
        return 2;
    if (y_stricmp(name, "SIDEDEFS") == 0)
        return 3;
    if (y_stricmp(name, "VERTEXES") == 0)
        return 4;
    if (y_stricmp(name, "SECTORS") == 0)
        return 5;
...
{
    if (y_stricmp(name, "SEGS") == 0)
        return true;
    if (y_stricmp(name, "SSECTORS") == 0)
        return true;
    if (y_stricmp(name, "NODES") == 0)
        return true;
    if (y_stricmp(name, "REJECT") == 0)
        return true;
    if (y_stricmp(name, "BLOCKMAP") == 0)
        return true;
    if (y_stricmp(name, "BEHAVIOR") == 0)
        return true;
    if (y_stricmp(name, "SCRIPTS") == 0)
        return true;
...
bool Wad_file::ReadDirectory()
{
    ...
    for (short i = 0; i < dir_count; i++)
    {
        ...
        if (lump->l_length != 0)
        {
            ...
            if (lump->l_length < 0 || lump->l_start < 0 || lump->l_length >= max_size || lump->l_start > total_size ||
                lump->l_start + lump->l_length > total_size)
            {
                LogPrintf("WARNING: clearing lump '%s' with invalid position (%d+%d > %d)\n", lump->name, lump->l_start,
                          lump->l_length, total_size);
...
static bool IsDummyMarker(const char *name)
{
    ...
    if (y_stricmp(name + 2, "_START") == 0 || y_stricmp(name + 2, "_END") == 0)
        return true;
...
bool Wad_file::WasExternallyModified()
{
    ...
    if (dir_start != LE_S32(header.dir_start) || dir_count != LE_S32(header.num_entries))
        return true;
...
void Wad_file::FixGroup(std::vector<short> &group, short index, short num_added, short num_removed)
{
...
int W_LoadLumpData(Lump_c *lump, byte **buf_ptr)
{
    ...
    *buf_ptr = new byte[lump->Length() + 1];
    ...
    if (lump->Length() > 0)
    {
        if (!lump->Seek() || !lump->Read(*buf_ptr, lump->Length()))
