...
class line_flag_CB_data_c
{
  ...
  public:
    line_flag_CB_data_c(UI_LineBox *_parent, int _mask) : parent(_parent), mask(_mask)
    {
    }
...
void UI_LineBox::SetTexOnLine(int ld, int new_tex, int e_state, int parts)
{
    ...
    if (e_state & FL_BUTTON2)
    {
        ...
        if (!(L->flags & MLF_LowerUnpegged) && is_null_tex(L->Right()->MidTex()) && is_null_tex(L->Left()->MidTex()))
        {
...
void UI_LineBox::CB_Copy(int parts)
{
    ...
    for (int pass = 0; pass < 2; pass++)
    {
        ...
        for (int b = 0; b < 3; b++)
        {
            ...
            const char *b_name = (b == 0) ? SD->l_tex->value() : (b == 1) ? SD->u_tex->value() : SD->r_tex->value();
            SYS_ASSERT(b_name);
...
int UI_LineBox::CalcFlags() const
{
    ...
    case 0: /* Normal    */;
        break;
    case 1: /* Invisible */
        lineflags |= MLF_DontDraw;
        break;
    case 2: /* Mapped    */
        lineflags |= MLF_Mapped;
        break;
    case 3: /* Secret    */
        lineflags |= MLF_Secret;
        break;
    }

    if (f_upper->value())
        lineflags |= MLF_UpperUnpegged;
    if (f_lower->value())
        lineflags |= MLF_LowerUnpegged;

    if (f_walk->value())
        lineflags |= MLF_Blocking;
    if (f_mons->value())
        lineflags |= MLF_BlockMonsters;
    if (f_sound->value())
        lineflags |= MLF_SoundBlock;
