...
Grid_State_c::Grid_State_c()
    : step(64 /* dummy */), snap(true), ratio(0), shown(true), orig_x(0.0), orig_y(0.0), Scale(1.0)
{
}
...
{
    // no change?
    if (fabs(x - orig_x) < 0.01 && fabs(y - orig_y) < 0.01)
        return;
...
void Grid_State_c::RatioSnapXY(double &var_x, double &var_y, double start_x, double start_y) const
{
...
const double Grid_State_c::scale_values[] = {
    ...
    1.0 / 1.5, 1.0 / 2.0, 1.0 / 3.0, 1.0 / 4.0, 1.0 / 6.0, 1.0 / 8.0, 1.0 / 16.0, 1.0 / 32.0, 1.0 / 64.0};
...
{
    fprintf(fp, "map_pos %1.0f %1.0f %1.6f\n", grid.orig_x, grid.orig_y, grid.Scale);

    fprintf(fp, "grid %d %d %d\n", grid.shown ? 1 : 0, grid_style ? 0 : 1, /* was grid.mode, now unused */
            grid.step);
