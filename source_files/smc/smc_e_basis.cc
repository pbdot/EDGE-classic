...
#include "smc_m_game.h" // g_default_xxx
...
const char *NameForObjectType(obj_type_e type, bool plural)
{
    ...
    {
    case OBJ_THINGS:
        return plural ? "things" : "thing";
    case OBJ_LINEDEFS:
        return plural ? "linedefs" : "linedef";
    case OBJ_SIDEDEFS:
        return plural ? "sidedefs" : "sidedef";
    case OBJ_VERTICES:
        return plural ? "vertices" : "vertex";
    case OBJ_SECTORS:
        return plural ? "sectors" : "sector";
...
{
    return (left >= 0) && (right >= 0) && SideDefs[left]->sector == SideDefs[right]->sector;
}
...
bool BA_ChangeSD(int side, byte field, int value)
{
    ...
    if (field == SideDef::F_LOWER_TEX || field == SideDef::F_UPPER_TEX || field == SideDef::F_MID_TEX)
    {
