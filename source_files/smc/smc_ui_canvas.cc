...
#include "smc_e_path.h" // SoundPropagation
...
#endif
#endif
      last_highlight(), last_splitter(-1), last_split_x(), last_split_y(), snap_x(-1), snap_y(-1), seen_sectors()
{
#ifdef NO_OPENGL
    rgb_buf = NULL;
#endif
...
void UI_Canvas::DrawLinedefs()
{
    ...
    for (int n = 0; n < NumLineDefs; n++)
    {
        ...
        case OBJ_VERTICES: {
            ...
            if (n != edit.split_line.num && n >= (NumLineDefs - 4) && !edit.show_object_numbers)
            {
...
void UI_Canvas::DrawLineInfo(double map_x1, double map_y1, double map_x2, double map_y2, bool force_ratio)
{
    ...
    if (mx < map_lx + 4 || mx > map_hx - 4 || my < map_ly + 4 || my > map_hy - 4)
    {
...
//
void UI_Canvas::DrawHighlight(int objtype, int objnum, bool skip_lines, double dx, double dy)
{
    ...
    case OBJ_SECTORS: {
        for (int n = 0; n < NumLineDefs; n++)
        {
            ...
            if (skip_lines && L->TwoSided())
            {
                ...
                if ((sec1 == objnum || edit.Selected->get(sec1)) && (sec2 == objnum || edit.Selected->get(sec2)))
                    continue;
...
{
    RenderLine(SCREENX(map_x1), SCREENY(map_y1), SCREENX(map_x2), SCREENY(map_y2));
}

//
//  draw a line with a "knob" showing the right (front) side
//
void UI_Canvas::DrawKnobbyLine(double map_x1, double map_y1, double map_x2, double map_y2, bool reverse)
{
...
void UI_Canvas::DrawCamera()
{
    ...
    // notches on body
    DrawMapLine(mx - dy * 0.4, my + dx * 0.4, mx + dy * 0.4, my - dx * 0.4);
...
void UI_Canvas::DrawCurrentLine()
{
    ...
    FindCrossingPoints(cross, V->x(), V->y(), edit.draw_from.num, new_x, new_y,
                       edit.highlight.valid() ? edit.highlight.num : -1);
...
void UI_Canvas::DragDelta(double *dx, double *dy)
{
    ...
    *dy = edit.drag_cur_y - edit.drag_start_y;
    ...
    if (edit.mode == OBJ_VERTICES && grid.ratio > 0 && edit.dragged.num >= 0 && edit.drag_other_vert >= 0)
    {
        const Vertex *v0 = Vertices[edit.drag_other_vert];
        const Vertex *v1 = Vertices[edit.dragged.num];
...
void UI_Canvas::RenderSector(int num)
{
    ...
    else if (edit.sector_render_mode == SREND_SoundProp)
    {
        ...
        case PGL_Never:
            return;
        case PGL_Maybe:
            RenderColor(fl_rgb_color(64, 64, 192));
            break;
        case PGL_Level_1:
            RenderColor(fl_rgb_color(192, 32, 32));
            break;
        case PGL_Level_2:
            RenderColor(fl_rgb_color(192, 96, 32));
            break;
        }
#endif
    }
    else
    {
        ...
        if (edit.sector_render_mode == SREND_Ceiling || edit.sector_render_mode == SREND_CeilBright)
...
{
    return ((y < 0) ? O_TOP : 0) | ((y >= rgb_h) ? O_BOTTOM : 0) | ((x < 0) ? O_LEFT : 0) |
           ((x >= rgb_w) ? O_RIGHT : 0);
}
#endif // NO_OPENGL
