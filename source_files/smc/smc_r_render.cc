...
Render_View_t::Render_View_t()
    : p_type(0), px(), py(), x(), y(), z(), angle(), Sin(), Cos(), screen_w(), screen_h(), screen(NULL),
      texturing(false), sprites(false), lighting(false), gravity(true), thing_sectors(), mouse_x(-1), mouse_y(-1)
{
}
...
{
    if (type == OBJ_THINGS && (field == Thing::F_X || field == Thing::F_Y))
    {
        thing_sec_cache::InvalidateThing(objnum);
    }
}
...
class save_obj_field_c
{
  ...
  public:
    save_obj_field_c(int _obj, int _field, int _value) : obj(_obj), field(_field), value(_value)
    {
    }
...
void AdjustOfs_CalcDistFactor(float &dx_factor, float &dy_factor)
{
    ...
    float dx = (r_view.x < edit.adjust_bbox.x1)   ? (edit.adjust_bbox.x1 - r_view.x)
               : (r_view.x > edit.adjust_bbox.x2) ? (r_view.x - edit.adjust_bbox.x2)
                                                  : 0;

    float dy = (r_view.y < edit.adjust_bbox.y1)   ? (edit.adjust_bbox.y1 - r_view.y)
               : (r_view.y > edit.adjust_bbox.y2) ? (r_view.y - edit.adjust_bbox.y2)
                                                  : 0;
    ...
    if (!edit.Selected->empty())
    {
        for (sel_iter_c it(edit.Selected); !it.done(); it.next())
        {
            ...
            if (parts & PART_RT_LOWER)
                AdjustOfs_Add(ld_num, PART_RT_LOWER);
            if (parts & PART_RT_UPPER)
                AdjustOfs_Add(ld_num, PART_RT_UPPER);
            if (parts & PART_RT_RAIL)
                AdjustOfs_Add(ld_num, PART_RT_RAIL);

            if (parts & PART_LF_LOWER)
                AdjustOfs_Add(ld_num, PART_LF_LOWER);
            if (parts & PART_LF_UPPER)
                AdjustOfs_Add(ld_num, PART_LF_UPPER);
            if (parts & PART_LF_RAIL)
                AdjustOfs_Add(ld_num, PART_LF_RAIL);
        }
    }
...
void Render3D_UpdateHighlight()
{
    ...
    if (edit.pointer_in_window && r_view.mouse_x >= 0 && edit.action != ACT_DRAG)
    {
        Objid current_hl;
...
static int LD_GrabTex(const LineDef *L, int part)
{
    ...
    if (part & PART_RT_UPPER)
        return L->Right()->upper_tex;

    if (part & PART_LF_LOWER)
        return L->Left()->lower_tex;
    if (part & PART_LF_UPPER)
        return L->Left()->upper_tex;

    if (part & PART_RT_RAIL)
        return L->Right()->mid_tex;
    if (part & PART_LF_RAIL)
        return L->Left()->mid_tex;
...
void Render3D_WriteUser(FILE *fp)
{
    ...
    fprintf(fp, "r_modes %d %d %d\n", r_view.texturing ? 1 : 0, r_view.sprites ? 1 : 0, r_view.lighting ? 1 : 0);
...
static editor_command_t render_commands[] = {
    ...
    {"3D_Toggle", NULL, &R3D_Toggle,
     /* flags */ NULL,
     /* keywords */ "tex obj light grav"},

    {"3D_Forward", NULL, &R3D_Forward},

    {"3D_Backward", NULL, &R3D_Backward},

    {"3D_Left", NULL, &R3D_Left},

    {"3D_Right", NULL, &R3D_Right},

    {"3D_Up", NULL, &R3D_Up},

    {"3D_Down", NULL, &R3D_Down},

    {"3D_Turn", NULL, &R3D_Turn},

    {"3D_DropToFloor", NULL, &R3D_DropToFloor},

    {"3D_ACT_AdjustOfs", NULL, &R3D_ACT_AdjustOfs},

    {"3D_WHEEL_Move", NULL, &R3D_WHEEL_Move},

    {"3D_NAV_Forward", NULL, &R3D_NAV_Forward},

    {"3D_NAV_Back", NULL, &R3D_NAV_Back},

    {"3D_NAV_Right", NULL, &R3D_NAV_Right},

    {"3D_NAV_Left", NULL, &R3D_NAV_Left},

    {"3D_NAV_Up", NULL, &R3D_NAV_Up},

    {"3D_NAV_Down", NULL, &R3D_NAV_Down},

    {"3D_NAV_TurnLeft", NULL, &R3D_NAV_TurnLeft},

    {"3D_NAV_TurnRight", NULL, &R3D_NAV_TurnRight},

    // backwards compatibility.
    // [ we cannot remap this in FindEditorCommand, it fails the context check ]
    {"3D_Align", NULL, &CMD_LIN_Align,
