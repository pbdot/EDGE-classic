...
static int DialogShowAndRun(char icon_type, const char *message, const char *title, const char *link_title = NULL,
                            const char *link_url = NULL, std::vector<std::string> *labels = NULL)
{
    dialog_result = -1;
    ...
    if (link_title)
    {
        ...
        UI_HyperLink *link = new UI_HyperLink(ICON_W + 20, 10 + mesg_H, mesg_W, 24, link_title, link_url);
    ...
    {
        const char *text = labels ? (*labels)[b].c_str() : (icon_type == '?') ? "OK" : "Close";
...
static void ParseButtons(const char *buttons, std::vector<std::string> &labels)
{
    for (;;)
    {
