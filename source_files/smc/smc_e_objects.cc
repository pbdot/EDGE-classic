...
static bool CheckClosedLoop(int new_ld, int v1, int v2, selection_c *flip)
{
    ...
    if (!left.loop.faces_outward)
        left.loop.FindIslands();
    if (!right.loop.faces_outward)
        right.loop.FindIslands();
    ...
    if (right.loop.faces_outward && right.sec >= 0 && left.loop.AllBare() && left.loop.islands.empty())
    {
        return true;
    }
...
static void Insert_LineDef(int v1, int v2, bool no_fill = false)
{
    ...
    if (Vertex_HowManyLineDefs(v1) >= 2 && Vertex_HowManyLineDefs(v2) >= 2)
    {
...
static void Insert_LineDef_autosplit(int v1, int v2, bool no_fill = false)
{
    ...
    FindCrossingPoints(cross, Vertices[v1]->x(), Vertices[v1]->y(), v1, Vertices[v2]->x(), Vertices[v2]->y(), v2);
...
static void Insert_Vertex(bool force_continue, bool no_fill)
{
    ...
    if (split_ld >= 0)
    {
        ...
        if (old_vert >= 0 && LineDefs[split_ld]->TouchesVertex(old_vert))
        {
            old_vert = -1;
        }
    }
    else
    {
        ...
        if (new_vert >= 0)
        {
            ...
            if (old_vert >= 0 && Vertices[old_vert]->Matches(Vertices[new_vert]))
            {
    ...
    if (new_vert < 0 && old_vert >= 0 && Vertices[old_vert]->Matches(MakeValidCoord(new_x), MakeValidCoord(new_y)))
    {
        edit.Selected->set(old_vert);
        return;
    }
...
begin_drawing:
    // begin drawing mode?
    if (edit.action == ACT_NOTHING && !closed_a_loop && old_vert >= 0 && new_vert < 0)
    {
        Selection_Clear();
    ...
    if (select_verts_of_new_sectors && closed_a_loop && NumSectors > orig_num_sectors)
    {
...
static void TransferLinedefProperties(int src_line, int dest_line, bool do_tex)
{
    ...
    if (do_tex && L1->Right() && L2->Right())
    {
        ...
        else if (!L2->Left())
        {
            ...
            if (is_null_tex(BA_GetString(f_l)))
                f_l = 0;
            if (is_null_tex(BA_GetString(f_u)))
                f_u = 0;
            if (is_null_tex(BA_GetString(b_l)))
                b_l = 0;
            if (is_null_tex(BA_GetString(b_u)))
                b_u = 0;

            // try hard to find a usable texture
            int tex = -1;

            if (front->floorh < back->floorh && f_l > 0)
                tex = f_l;
            else if (front->floorh > back->floorh && b_l > 0)
                tex = b_l;
            else if (front->ceilh > back->ceilh && f_u > 0)
                tex = f_u;
            else if (front->ceilh < back->ceilh && b_u > 0)
                tex = b_u;
            else if (f_l > 0)
                tex = f_l;
            else if (b_l > 0)
...
static void Drag_UpdateCurrentDist(int obj_type, int objnum, double *x, double *y, double *best_dist, double ptr_x,
                                   double ptr_y, bool only_grid)
{
    double x2, y2;
    ...
    case OBJ_LINEDEFS: {
        ...
        Drag_UpdateCurrentDist(OBJ_VERTICES, L->start, x, y, best_dist, ptr_x, ptr_y, only_grid);

        Drag_UpdateCurrentDist(OBJ_VERTICES, L->end, x, y, best_dist, ptr_x, ptr_y, only_grid);
    }
        return;
    ...
    case OBJ_SECTORS:
        ...
        for (int n = 0; n < NumLineDefs; n++)
        {
            ...
            Drag_UpdateCurrentDist(OBJ_LINEDEFS, n, x, y, best_dist, ptr_x, ptr_y, only_grid);
        }
        return;
...
void GetDragFocus(double *x, double *y, double ptr_x, double ptr_y)
{
    ...
    *y = 0;
    ...
    {
        Drag_UpdateCurrentDist(edit.mode, edit.dragged.num, x, y, &best_dist, ptr_x, ptr_y, only_grid);
        return;
    }

    for (sel_iter_c it(edit.Selected); !it.done(); it.next())
    {
        Drag_UpdateCurrentDist(edit.mode, *it, x, y, &best_dist, ptr_x, ptr_y, only_grid);
    }
}
...
static void DoRotate90Things(selection_c *list, bool anti_clockwise, double mid_x, double mid_y)
{
    fixcoord_t fix_mx = MakeValidCoord(mid_x);
    fixcoord_t fix_my = MakeValidCoord(mid_y);
...
void ScaleObjects4(double scale_x, double scale_y, double scale_z, double pos_x, double pos_y, double pos_z)
{
