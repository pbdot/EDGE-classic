...
#include "smc_e_cutpaste.h" // Texboard_XX
#include "smc_e_path.h"     // GoToObject
#include "smc_e_main.h"     // Selection_Clear
#include "smc_m_config.h"   // gui_scheme
...
#endif
      cur_obj(), find_numbers(new number_group_c), tag_numbers(new number_group_c)
{
#ifdef _FLTK_DISABLED
    box(FL_FLAT_BOX);
...
void UI_FindAndReplace::UpdateWhatColor()
{
    ...
    {
    case 0: /* Things      */
        what->color(THING_MODE_COL);
        break;
    case 1: /* Line Tex    */
        what->color(LINE_MODE_COL);
        break;
    case 2: /* Sector Flat */
        what->color(SECTOR_MODE_COL);
        break;
    case 3: /* Line Type   */
        what->color(FL_GREEN);
        break;
    case 4: /* Sector Type */
        what->color(fl_rgb_color(255, 160, 0));
        break;
    }
#endif
}
...
void UI_FindAndReplace::what_kind_callback(Fl_Widget *w, void *data)
{
    ...
    {
    case 0:
        box->cur_obj.type = OBJ_THINGS;
        break;
    case 1:
        box->cur_obj.type = OBJ_LINEDEFS;
        want_descs        = false;
        break;
    case 2:
        box->cur_obj.type = OBJ_SECTORS;
        want_descs        = false;
        break;
    case 3:
        box->cur_obj.type = OBJ_LINEDEFS;
...
void UI_FindAndReplace::find_match_callback(Fl_Widget *w, void *data)
{
    ...
    bool is_valid = box->CheckInput(box->find_match, box->find_desc, box->find_pic, box->find_numbers);
...
bool UI_FindAndReplace::CheckInput(Fl_Input *w, Fl_Output *desc, UI_Pic *pic, number_group_c *num_grp)
{
    ...
    {
        int mask = (Features.gen_sectors == 2) ? 255 : (Features.gen_sectors) ? 31 : 65535;
...
void UI_FindAndReplace::CB_Copy(bool is_replace)
{
    ...
    if (tex_name[0])
    {
        bool is_known = (what->value() == 1) ? W_TextureIsKnown(tex_name) : W_FlatIsKnown(tex_name);
...
void UI_FindAndReplace::CB_Paste(bool is_replace)
{
    ...
    int tex_num = (what->value() == 1) ? Texboard_GetTexNum() : Texboard_GetFlatNum();
...
void UI_FindAndReplace::BrowsedItem(char kind, int number, const char *name, int e_state)
{
    ...
    int sel_pics = find_pic->Selected() ? 1 : rep_pic->Selected() ? 2 : 0;

    if (sel_pics == 0)
    {
        sel_pics = (Fl::focus() == find_match || Fl::focus() == find_desc) ? 1
                   : (Fl::focus() == rep_value || Fl::focus() == rep_desc) ? 2
                                                                           : 0;
    }
...
bool UI_FindAndReplace::FindNext()
{
    ...
    if (is_first)
    {
        ...
        if (filter_toggle->value() && restrict_to_sel->value() && edit.Selected->empty())
        {
...
void UI_FindAndReplace::DoReplace()
{
    ...
    if (strlen(find_match->value()) == 0 || strlen(rep_value->value()) == 0)
    {
        Beep("Bad replace");
        return;
    }
...
bool UI_FindAndReplace::Match_Sector(int idx)
{
    ...
    if (!filter_toggle->value() || (!is_sky(ceil_tex) && o_ceilings->value()) || (is_sky(ceil_tex) && o_skies->value()))
