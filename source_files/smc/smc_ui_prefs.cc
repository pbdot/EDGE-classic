...
class UI_EditKey : public UI_Escapable_Window
{
  ...
  private:
    ...
    int handle(int event)
    {
        ...
        if (awaiting_key)
        {
            ...
            if (event == FL_KEYDOWN || event == FL_PUSH || event == FL_MOUSEWHEEL)
            {
  ...
  private:
    ...
    void Decode(key_context_e ctx, const char *str)
    {
        ...
        while (*str && !(isspace(*str) || *str == ':' || *str == '/') && pos + 4 < sizeof(func_buf))
        {
            func_buf[pos++] = *str++;
        }
  ...
  public:
    UI_EditKey(keycode_t _key, key_context_e ctx, const char *_funcname)
        : UI_Escapable_Window(400, 306, "Edit Key Binding"), want_close(false), cancelled(false), awaiting_key(false),
          key(_key)
    {
    ...
    bool Run(keycode_t *key_v, key_context_e *ctx_v, const char **func_v, bool start_grabbed)
    {
#ifdef _FLTK_DISABLED
        *key_v  = 0;
        *ctx_v  = KCTX_NONE;
...
#endif
      want_quit(false), want_discard(false), key_sort_mode('k'), key_sort_rev(false), awaiting_line(0)
{
#ifdef _FLTK_DISABLED
    if (gui_color_set == 2)
...

    {
...

        {
            ...
            {
                gen_swapsides =
                    new Fl_Check_Button(50, 310, 380, 25, " swap upper and lower sidedefs in Linedef panel");
            }
            {
