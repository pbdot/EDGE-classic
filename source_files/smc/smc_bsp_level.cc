...
void CheckLimits(bool &force_v5, bool &force_xnod)
{
    ...
    {
        if (num_old_vert > 32767 || num_new_vert > 32767 || num_segs > 65534 || num_nodes > 32767)
        {
...
static u32_t CalcGLChecksum(void)
{
    ...
    if (lump && lump->Length() > 0)
    {
        ...
        if (!lump->Seek() || !lump->Read(data, lump->Length()))
...
static const char *CalcOptionsString()
{
    ...
    snprintf(buffer, sizeof(buffer), "--cost %d%s", cur_info->factor, cur_info->fast ? " --fast" : "");
