...
void RenderCurrentUnits(void)
{
    ...
    const SokolRenderState *fixme = (SokolRenderState *)global_render_state;
    ...
    for (int j = 0; j < current_render_unit; j++)
    {
        ...
        sgl_enable_texture();
        sgl_load_pipeline(fixme->pip_3d_default_);

        if (unit->blending & kBlendingAlpha)
        {
            sgl_load_pipeline(fixme->pip_3d_alpha_);
        }

        if (unit->blending & kBlendingAdd)
        {
            sgl_load_pipeline(fixme->pip_3d_add_);
        }
