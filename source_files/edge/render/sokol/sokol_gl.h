...
SOKOL_GL_API_DECL void sgl_enable_ssao(void);
SOKOL_GL_API_DECL void sgl_disable_ssao(void);
...
SOKOL_GL_API_DECL void sgl_set_clipplane_enabled(int clipplane, bool enabled);
...
\
    void main()\
    {\
        vec4 c0 = texture(tex0_smp0, uv.xy) * color;\
...
static void _sgl_setup_common(void) {
    ...
    shd_desc.images[1].stage = SG_SHADERSTAGE_FRAGMENT;
    shd_desc.images[1].image_type = SG_IMAGETYPE_2D;
    shd_desc.images[1].sample_type = SG_IMAGESAMPLETYPE_FLOAT;
    shd_desc.images[1].hlsl_register_t_n = 0; // ???
    shd_desc.images[1].msl_texture_n = 0; // ???
    shd_desc.images[1].wgsl_group1_binding_n = 64; // ???
    ...
    shd_desc.samplers[1].hlsl_register_s_n = 0;
    shd_desc.samplers[1].msl_sampler_n = 0;
    shd_desc.samplers[1].wgsl_group1_binding_n = 80;
    ...
    shd_desc.image_sampler_pairs[0].glsl_name = "tex0_smp0";

    shd_desc.image_sampler_pairs[1].stage = SG_SHADERSTAGE_FRAGMENT;
    shd_desc.image_sampler_pairs[1].image_slot = 1;
    shd_desc.image_sampler_pairs[1].sampler_slot = 1;
    shd_desc.image_sampler_pairs[1].glsl_name = "tex1_smp1";
...
SOKOL_API_IMPL void sgl_defaults(void) {
    ...
    SOKOL_ASSERT(!ctx->in_begin);
    ctx->u0 = 0.0f; ctx->v0 = 0.0f;
    ctx->u1 = 0.0f; ctx->v1 = 0.0f;
    ctx->rgba = 0xFFFFFFFF;
    ctx->point_size = 1.0f;
...
SOKOL_API_IMPL void sgl_texture(sg_image img0, sg_sampler smp0) {
    ...
    SOKOL_ASSERT(!ctx->in_begin);
    if (SG_INVALID_ID != img0.id) {
        ctx->cur_img0 = img0;
    } else {
        ctx->cur_img0 = _sgl.def_img;
    }
    if (SG_INVALID_ID != smp0.id) {
        ctx->cur_smp0 = smp0;
    } else {
        ctx->cur_smp0 = _sgl.def_smp;
    }
...
SOKOL_API_IMPL void sgl_multi_texture(sg_image img0, sg_sampler smp0, sg_image img1, sg_sampler smp1) {
    ...
    if (SG_INVALID_ID != img1.id) {
        ctx->cur_img1 = img1;
    } else {
        ctx->cur_img1 = _sgl.def_img;
    }
    if (SG_INVALID_ID != smp1.id) {
        ctx->cur_smp1 = smp1;
    } else {
        ctx->cur_smp1 = _sgl.def_smp;
    }
...
SOKOL_API_IMPL void sgl_enable_ssao(void) {
    ...
    _sgl_fragment_uniform_t *cf = &ctx->fragment_uniforms.ptr[ctx->fragment_uniforms.next];
    if (!(cf->flags & SGL_STATE_SSAO))
    {   
        cf->flags |= SGL_STATE_SSAO;
        ctx->fragment_uniforms_dirty = true;
    }    
}

SOKOL_API_IMPL void sgl_disable_ssao(void) {
    SOKOL_ASSERT(_SGL_INIT_COOKIE == _sgl.init_cookie);
    _sgl_context_t* ctx = _sgl.cur_ctx;
    if (!ctx) {
        return;
...
SOKOL_API_IMPL void sgl_end(void) {
    ...
    if (vertex_uniforms_dirty) {
        ...
        if (uni) {
            ...
            memcpy(uni->clipplanes, ctx->clipplanes, sizeof(float) * 24);
        }
    }
    ...
    sg_image img0 = ctx->texturing_enabled ? ctx->cur_img0 : _sgl.def_img;
    sg_sampler smp0 = ctx->texturing_enabled ? ctx->cur_smp0 : _sgl.def_smp;
    sg_image img1 = ctx->texturing_enabled ? ctx->cur_img1 : _sgl.def_img;
    sg_sampler smp1 = ctx->texturing_enabled ? ctx->cur_smp1 : _sgl.def_smp;
    ...
    if (cur_cmd) {
        if ((cur_cmd->cmd == SGL_COMMAND_DRAW) &&
            ...
            (cur_cmd->args.draw.smp1.id == smp1.id) &&
    ...
    if (merge_cmd) {
        ...
        if (cmd) {
            ...
            cmd->args.draw.smp0 = smp0;
            cmd->args.draw.img1 = img1;
            cmd->args.draw.smp1 = smp1;
...
SOKOL_API_IMPL void sgl_set_clipplane(int clipplane, float x, float y, float z, float d)
{
    SOKOL_ASSERT(clipplane >= 0 && clipplane < 6);
    SOKOL_ASSERT(_SGL_INIT_COOKIE == _sgl.init_cookie);
    ...
    _sgl_matrix_t* mv = _sgl_matrix_modelview(ctx);

    float r[16];
    const float* m = &mv->v[0][0];
    ...
    ctx->clipplanes[clipplane][0] = r[0] * x + r[1] * y + r[2] * z + r[3] * d;
    ctx->clipplanes[clipplane][1] = r[4] * x + r[5] * y + r[6] * z + r[7] * d;
    ctx->clipplanes[clipplane][2] = r[8] * x + r[9] * y + r[10] * z + r[11] * d;
    ctx->clipplanes[clipplane][3] = r[12] * x + r[13] * y + r[14] * z + r[15] * d;

    ctx->vertex_uniforms_dirty = true;
}

SOKOL_API_IMPL void sgl_set_clipplane_enabled(int clipplane, bool enabled)
{
    SOKOL_ASSERT(_SGL_INIT_COOKIE == _sgl.init_cookie);
    _sgl_context_t *ctx = _sgl.cur_ctx;
    if (ctx)
    {
        ...
        bool dirty = false;
        int32_t clipplanes = cf->clipplanes;
        int32_t plane = 1 << clipplane;
        if (!enabled)
        {
            if (!(clipplanes & plane))
            {
        ...
        {
            ctx->fragment_uniforms_dirty = true;
            cf->clipplanes                    = clipplanes;
        }
    }
}

SOKOL_API_IMPL void sgl_set_alpha_test(float alpha_test)
{
    SOKOL_ASSERT(_SGL_INIT_COOKIE == _sgl.init_cookie);
    _sgl_context_t *ctx = _sgl.cur_ctx;
    if (ctx)
    {
        _sgl_fragment_uniform_t *cf = &ctx->fragment_uniforms.ptr[ctx->fragment_uniforms.next];
        if (cf->alpha_test != alpha_test)
        {
            ctx->fragment_uniforms_dirty = true;
            cf->alpha_test = alpha_test;
        }
    }
}
