    ...
    if (!ttf_buffer)
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            stbtt_GetPackedQuad(ttf_atlas[i]->chardata_for_range, res_font_bitmap_sizes[i], res_font_bitmap_sizes[i],
                                static_cast<u8_t>(ch), &x, &y, ref.char_quad[i], 0);
        ...
        else
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                stbtt_GetPackedQuad(ttf_atlas[i]->chardata_for_range, res_font_bitmap_sizes[i],
                                    res_font_bitmap_sizes[i], static_cast<u8_t>(ch), &x, &y, character.char_quad[i], 0);
                if (ch == ' ')
...
int font_c::GetGlyphIndex(char ch)
{
    ...
    else
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            float y                = 0.0f;
            stbtt_GetPackedQuad(ttf_atlas[i]->chardata_for_range, res_font_bitmap_sizes[i], res_font_bitmap_sizes[i],
                                static_cast<u8_t>(ch), &x, &y, character.char_quad[i], 0);
            if (ch == ' ')
                character.width[i] = ttf_char_width[i] * 3 / 5;
            else
                character.width[i] =
